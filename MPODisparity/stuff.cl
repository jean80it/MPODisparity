...
__kernel void scaleUpLinHPrism(__global float * data, uint stride, uint offs, int w)
{
    const uint X = get_global_id(0);
    const uint Y = get_global_id(1);
...
__kernel void scaleDownHPrism(__global float * data, uint stride, uint offs, int w)
{
    const uint X = get_global_id(0);
    const uint Y = get_global_id(1);
...
//		| 6 8 | 6 7 ||1 2 | 1 3 | 1 4 | 1 5 | 1 6 | 1 7 | 1 8 |    |
//		| 5 8 | 5 7 | 5 6 ||2 3 | 2 4 | 2 5 | 2 6 | 2 7 | 2 8 |    |  workload size 2 (Z) = img W / 2
//		| 4 8 | 4 7 | 4 6 | 4 5 ||3 4 | 3 5 | 3 6 | 3 7 | 3 8 |    v
//		            ^ 
//		            |                 ||4 5 | 4 6 | 4 7 | 4 8 |
//		             ----------             ||5 6 | 5 7 | 5 8 |
//		                                          ||6 7 | 6 8 |
...
//		
// 
// a couple is (right x, left x) = (Z, X)
//
...
__kernel void blockMatch3x3W(
	...
	__global  uint * outDisparity, 
	uint outOffsDisparity,
	uint outStrideDisparity, 
	
	//__global uint * inPrevDisparity, 
	//uint inStridePrevDisparity, 
	//uint inOffsPrevDisparity,
	
	uint N) // N should be W-1
{
    const uint X = get_global_id(0);
    const uint Y = get_global_id(1);
	...
	const uint inPosR = inOffsR + Rx + (Y * inStrideR) - 1; // "-1" because of the 3x3 neighbourhood 
    const uint inPosL = inOffsL + Lx + (Y * inStrideL) - 1;
    ...
    //uint oldErrDisp = inPrevDisparity[inOffsPrevDisparity + (X / 2) + (Y * inStridePrevDisparity)];
    ...
    uint outPos = outOffsDisparity + Z + (Y * outStrideDisparity);
    
	atomic_min(outDisparity + outPos,  // weighted sum of absolute differences
                        /*err +*/ convert_ushort_sat(dot(fabs(br[0]-bl[0]),W[0])+
